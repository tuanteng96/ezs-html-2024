...
var EzsPrice = {
    ...
    onChangePM: (_this) => {
        ...
        let valueCs = Number($(".current-app-year").attr('data-price'))
        if(valueCs > 0) {
            let totalCs = Number($(".current-pm-cs").attr('data-cs'))
            if(totalCs > 1) {
                let vals = totalCs - 1
                $('.total-app').attr('data-totalapp', valueCs + (vals * 1000000)).html(`${EzsPrice.formatVND(valueCs + (vals * 1000000))}₫`)
            }
            else {
                $('.total-app').attr('data-totalapp', valueCs).html(`${EzsPrice.formatVND(valueCs)}₫`)
            }
        }
    ...
    onChangeAPP: _this => {
        ...
        let valueCs = Number($(".current-pm-cs").attr('data-cs'))
        if(valueCs > 1) {
            let vals = valueCs - 1
            $('.total-app').attr('data-totalapp', valPrice + (vals * 1000000)).html(`${EzsPrice.formatVND(valPrice + (vals * 1000000))}₫`)
        }
        else {
        ...
        if (valPrice > 0) {
            ...
            $('.total-app').attr('data-totalapp', 0)
...
const EzsListen = {
    ...
    getListen: () => {
        ...
        EzsListen.renderListen({
          file_mp3: "/mp3/sale.mp3",
        });
    },
    renderListen: (data) => {
...
const EzsVideo = {
    ...
    render: () => {
        let domVideo = $("#video-highlight");
        
        if (!domVideo || domVideo.length === 0) return;
        
        $(".popup-video-highlight").magnificPopup({
          //disableOn: 700,
          type: "iframe",
          mainClass: "mfp-fade",
          removalDelay: 160,
        ...
        // $.get("/wp-json/wp/v2/posts?categories=178", function(data) {
            
        //     if(data && data.length > 0) {
        //         for (let item of data) {
        //             let domItem = `<div class="col-md-4">
        //                     <a class="images popup-video-highlight" href="${item.acf.link_video}">
        //                         <img src="${item.featured_image_url}" class="w-100"/>
        //                     </a>
        //             </div>`
        //             domVideo.append(domItem)
        //         }
